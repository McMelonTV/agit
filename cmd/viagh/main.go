...
func run(argv []string) int {
	...
	if tool, ok := shimTool(argv[0]); ok {
		if tool == "gh" && !ghcmd.IsAuthStatus(argv[1:]) && !ghcmd.NeedsAuthentication(argv[1:]) {
	...
	if commandIndex, ok := rawTopLevelCommand(argv[1:]); ok && argv[1+commandIndex] == "gh" {
		...
		if !ghcmd.IsAuthStatus(ghArgs) && !ghcmd.NeedsAuthentication(ghArgs) {
...
func runTool(cfg config.Config, tool string, args []string) int {
	if tool == "gh" && !ghcmd.IsAuthStatus(args) && !ghcmd.NeedsAuthentication(args) {
	...
	if tool == "gh" && ghcmd.IsAuthStatus(args) {
		host := cfg.Host
		...
		if ghcmd.IsAuthStatusJSONHosts(args) {
			return runGitHubAppAuthStatus(cfg, host)
		}
		return runGitHubAppAuthStatusText(cfg, host)
	}
