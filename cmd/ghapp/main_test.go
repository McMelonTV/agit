...
`)
	...
	t.Setenv("GHAPP_PRIVATE_KEY_BASE64", "not-valid-base64")

	stdout, stderr, code := captureProcessOutput(t, func() int {
		return run([]string{"ghapp", "--real-gh", fakeGH, "gh", "--version"})
	})
...
`)
	...
	stdout, stderr, code := captureProcessOutput(t, func() int {
		return run([]string{"ghapp", "--real-gh=" + fakeGH, "gh", "repo", "list", "--help"})
	})
...
`)
	...
	t.Setenv("GHAPP_BROKER_URL", "http://127.0.0.1:1")
	t.Setenv("GHAPP_BROKER_SECRET", "secret")
	t.Setenv("GHAPP_SESSION_CONFIG", "session")
...
if [ -n "$GHAPP_ACTIVE_TOKEN_SHA256" ] || [ -n "$GHAPP_BROKER_URL" ] || [ -n "$GHAPP_BROKER_SECRET" ] || [ -n "$GHAPP_SESSION_CONFIG" ]; then
...
func TestCommandGroupHelpBypassesMalformedConfiguration(t *testing.T) {
	...
	t.Setenv("GHAPP_PRIVATE_KEY_BASE64", "not-valid-base64")
	stdout, stderr, code := captureProcessOutput(t, func() int {
		return run([]string{"ghapp", "--real-gh", fakeGH, "gh", "repo"})
	})
...
func TestTopLevelVersionBypassesConfiguration(t *testing.T) {
	t.Setenv("GHAPP_PRIVATE_KEY_BASE64", "not-valid-base64")
	stdout, stderr, code := captureProcessOutput(t, func() int {
		return run([]string{"ghapp", "--version"})
	})
	if code != 0 || stderr != "" || !strings.HasPrefix(stdout, "ghapp ") {
		t.Fatalf("code=%d stdout=%q stderr=%q", code, stdout, stderr)
	}
}

func TestTopLevelHelpAndVersionWordsBypassInvalidConfiguration(t *testing.T) {
	t.Setenv("GHAPP_INSTALLATION_ID", "not-a-number")
	for _, command := range []string{"help", "--help", "-h", "version"} {
		stdout, stderr, code := captureProcessOutput(t, func() int {
			return run([]string{"ghapp", command})
		})
...
func TestRestrictedUtilityCommandReportsMissingBroker(t *testing.T) {
	t.Setenv("GHAPP_BROKER_URL", "")
	t.Setenv("GHAPP_BROKER_SECRET", "")
...
func TestMalformedBase64PrivateKeyIsLazy(t *testing.T) {
	...
	if _, err := backend.appClient(); err == nil || !strings.Contains(err.Error(), "decode GHAPP_PRIVATE_KEY_BASE64") {
...
`)
	...
	t.Setenv("GHAPP_PRIVATE_KEY_BASE64", "not-valid-base64")
	stdout, stderr, code := captureProcessOutput(t, func() int {
		return run([]string{"ghapp", "--real-git", fakeGit, "git", "--version"})
	})
